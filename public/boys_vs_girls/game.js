...
testModeBtn.addEventListener('click', () => {
    ...
    testInterval = setInterval(() => {
        ...
        const mockUser = {
            ...
            nickname: ' ' + Math.floor(Math.random() * 8),
            profilePictureUrl: ''
        };
...
function getPlayerRadius(score) {
    return Math.max(15, Math.sqrt(score) * 4);
}

function shootSpike(player, count = 1) {
    playSound('hit');
...
function electricShock(player, multiplier) {
    ...
    for (let [id, p] of players) {
        ...
        if (p.shieldTimer <= 0 && p.ghostTimer <= 0) {
            const damage = Math.min(p.score - 10, 100 * multiplier);
            if (damage > 0) {
                p.score -= damage;
                p.freezeTimer = 100;
                player.score += damage;
    ...
    if (actionType === 'grow_small') {
        ...
        for (let [id, p] of players) {
            if (id !== player.id && p.shieldTimer <= 0 && p.ghostTimer <= 0 && p.score > 15) {
                const dmg = Math.min(p.score - 10, 50 * multiplier);
                if (dmg > 0) {
                    p.score -= dmg;
                    player.score += dmg;
...
function endGame() {
    ...
    if (topThree.length === 0) {
        winnersListDiv.innerHTML = '<h3 style="color:white;">  </h3>';
        return;
    }
...
function update() {
    ...
    if (blackHole) {
        ...
        for (let p of playersArr) {
            if (p.shieldTimer <= 0 && p.ghostTimer <= 0) {
                ...
                if (dist < 50 && p.score > 10 && p.id !== blackHole.ownerId) {
                    const dmg = Math.min(p.score - 10, 50.0);
                    if(dmg > 0) {
                        p.score -= dmg;
        ...
        for (let p of playersArr) {
            if (p.id !== l.ownerId && p.shieldTimer <= 0 && p.ghostTimer <= 0 && p.score > 10) {
                ...
                if (dot > 0) {
                    ...
                    if (distToLine < getPlayerRadius(p.score)) {
                        const dmg = Math.min(p.score - 10, 50.0 * (l.multiplier || 1));
                        if(dmg > 0) {
                            p.score -= dmg;
        ...
        if (m.y >= m.targetY) {
            ...
            for (let p of playersArr) {
                if (p.shieldTimer <= 0 && p.ghostTimer <= 0) {
                    ...
                    if (dist < m.radius * 3) {
                        const rawDmg = Math.max(50, Math.min(500, (m.radius * 3 - dist) * 2)) * m.multiplier;
                        const damage = Math.min(p.score - 10, rawDmg);
                        if (damage > 0) {
                            p.score -= damage;
    ...
    for (let i = spikes.length - 1; i >= 0; i--) {
        ...
        for (let p of playersArr) {
            ...
            if (dist < r) {
                if (p.shieldTimer <= 0 && p.ghostTimer <= 0) {
                    const damage = Math.min(p.score - 10, 50);
                    if (damage > 0) {
                        p.score -= damage;
                    ...
                    createParticles(s.x, s.y, s.ownerColor, 10);
                    playSound('hit');
                    
                    addFloatingText(p.x, p.y - r, `-${damage}`, '#ff0000');
                    if (owner) addFloatingText(owner.x, owner.y - getPlayerRadius(owner.score), `+${damage}`, '#00ff00');
                }
                spikes.splice(i, 1);
                updateLeaderboard();
                break;
...
function draw() {
    ...
    if (!isGameOver) {
        for (let [id, p] of players) {
            ...
            ctx.shadowBlur = 4;
            ctx.fillText(displayName, p.x, p.y - r - 10);
            
            ctx.font = '14px Arial';
            ctx.fillStyle = '#ffd700';
            ctx.fillText(Math.floor(p.score), p.x, p.y + r + 20);
            ctx.shadowBlur = 0;
        }
